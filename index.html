...
<head>
    ...
    <script type="module">
        ...
        // Emote Data for Mockup - Added unique colors to simulate different image assets
        const emotes = {
            'lol-emote-1': { name: 'LOL Emote (Victory Dance)', text: 'VICTORY%20DANCE', color: 'FFD700' }, // Gold theme
            'lol-emote-2': { name: 'LOL Emote (Waving)', text: 'WAVING', color: '1E90FF' }, // Blue theme
            'lol-emote-3': { name: 'LOL Emote (Heart)', text: 'HEART', color: 'FF69B4' }, // Pink theme
            'lol-emote-4': { name: 'LOL Emote (Flex)', text: 'FLEX', color: '32CD32' }, // Green theme
        };
        ...
        // Function to generate the placeholder image URL with unique colors
        function getEmoteImageUrl(emoteValue) {
            const emote = emotes[emoteValue];
            const text = emote ? emote.text : 'EMOTE';
            const color = emote ? emote.color : '9400D3';
            // Placeholder URL: Increased size to 250x250 with dynamic color background
            return `https://placehold.co/250x250/${color}/ffffff?text=${text}`;
        }
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            claimForm.addEventListener('submit', async (e) => {
                ...
                const safetyMessage = `
                    <p class="text-lg font-semibold mb-3 text-red-400">🚨 ACCOUNT SAFETY WARNING 🚨</p>
...
<body class="bg-gray-900 text-gray-100 min-h-screen p-4 sm:p-8 flex items-center justify-center">
    ...
    <div class="w-full max-w-lg bg-gray-800 p-6 sm:p-8 rounded-2xl shadow-2xl border-2 border-red-600">
        ...
        <p class="text-sm text-center text-yellow-400 font-bold mb-6">
            <span class="bg-red-900 px-3 py-1 rounded-full">★ MOCKUP INTERFACE - READ SAFETY WARNING ★</span>
        </p>
        ...
        <div class="bg-red-950 p-4 rounded-xl mb-6 border-l-4 border-red-500 text-sm">
            <p class="font-bold text-red-300">⚠️ IMPORTANT SAFETY NOTICE</p>
